...
public class MemberView extends JFrame implements ActionListener {
	...
	private JLabel title, nameLabel, useridLabel, passwdLabel, ssnLabel, addrLabel;
	private JTextField nameText, useridText, passwdText, ssnText, addrText;
	...
	public void open() {
		...
		passwdLabel.setLocation(100, 200);
		...
		passwdText.setLocation(200, 200);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == submitButton) {
			...
			for(int i=0;i<count; i++) {
				...
				result.setPasswd(passwords[i]);
				result.setSsn(ssns[i]);
				result.setAddr(addrs[i]);
				memberService.add(result);
			}
			...
			JOptionPane.showMessageDialog(this, "아이디" + useridText.getText() + "," + passwdText.getText());
	...
	Member returnMember = memberService.detail(useridText.getText());
			if(returnMember != null) {
				...
				resultText.setText("해당 id가 존재하지 않습니다.");
			}
			
	/*	System.out.println("11");
		JOptionPane.showMessageDialog(this, "회원확인 :" + nameText.getText()+","+useridText.getText()
	+ ","+ passwdText.getText()+","+ssnText.getText()+"," + addrText.getText());
		
		Member okk = memberService.detail(nameText.getText());
		...
		nameText.setText("");
			...
			String newPassword = passwdText.getText();
			...
			deleteMember.setPasswd(passwdText.getText());
